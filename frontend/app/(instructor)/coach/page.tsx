          ...
          <div>
            ...
            <div className="space-y-4">
              {pendingExams.slice(0, 3).map(exam => (
                <div key={exam.id} className="bg-white border border-neutral-200 rounded-2xl p-6 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-[#0B2545] text-sm mb-1">{exam.Module?.title || 'Assessment'}</h4>
                      <p className="text-xs text-neutral-500">Student: {exam.student?.name || 'Unknown'} &bull; {new Date(exam.createdAt).toLocaleDateString()}</p>
                    </div>
                  </div>
                  <Link href={`/coach/assessments/${exam.id}`} className="bg-[#964B13] hover:bg-[#72360B] text-white px-5 py-2 rounded-full text-xs font-bold transition-colors shadow-sm text-center">
                    Review
                  </Link>
                </div>
              ))}
              ...
              {pendingExams.length === 0 && (
                <div className="bg-white rounded-2xl p-6 border border-neutral-200 shadow-sm relative z-10 text-center">
                  <p className="text-sm text-neutral-500 py-2">Tidak ada tugas tertunda.</p>
                </div>
              )}
            </div>
          </div>
