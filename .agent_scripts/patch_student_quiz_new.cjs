...
`
// We need to replace the old StudentQuiz string in the file.
const regex = /const StudentQuiz = \(\{ chapterId, subChapter \}.*?^};\s*$/ms;
code = code.replace(regex, newStudentQuiz);

fs.writeFileSync(f, code);
console.log('Patched StudentQuiz logic');
